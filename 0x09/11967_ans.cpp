...
int main(void) {
  ...
  while (!Q.empty()) {
    ...
    cout << nu++ << ") " << cur.X << "," << cur.Y << "\n";
    Q.pop();
    for (auto nxt : adj[cur.X][cur.Y]) { // cur에서 불 켤수 있는 곳
      if (vis[nxt.X][nxt.Y]) continue; // Q에 들어간 적 있?
      if (is_conn(nxt)) { // 상하좌우에 스위치로 불 켤 수 있다면 거기로 이동
        vis[nxt.X][nxt.Y] = 1;
        Q.push({nxt.X, nxt.Y});
      }
      board[nxt.X][nxt.Y] = 1; // switch 있어서 켤 수 있는 불은 다 킴
    }
    for (int dir = 0; dir < 4; dir++) {
      int nx = cur.X + dx[dir];
      int ny = cur.Y + dy[dir];
      if (OOB(nx, ny) || vis[nx][ny] || board[nx][ny] == 0) continue; // 아직 방문x, 불이 켜져있다면
      vis[nx][ny] = 1;
      Q.push({nx, ny});
    }
  }
